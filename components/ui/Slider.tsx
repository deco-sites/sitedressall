...
function Dot({ index, children }: {
  ...
  return (
    <button
      ...
      class="focus:outline-none group w-full h-fit"
    >
      {children}
    </button>
  );
  ...
  const observer = new IntersectionObserver(
    (elements) =>
      elements.forEach((item) => {
        ...
        const dotBannerCarrosel = dot?.querySelector<HTMLDivElement>(
          ".dot-banner-carrosel",
        );

        if (item.isIntersecting) {
          dot?.setAttribute("disabled", "");
          if (dotBannerCarrosel) {
            dotBannerCarrosel.style.backgroundColor = "red";
          }
        } else {
